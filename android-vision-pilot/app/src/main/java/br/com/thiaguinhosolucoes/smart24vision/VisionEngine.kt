...
class VisionEngine {
    ...
    suspend fun analyze(bitmap: Bitmap): VisionResult = withContext(Dispatchers.Default) {
        ...
        objects.forEachIndexed { index, obj ->
            ...
            val looksHuman = rect.height() >= 0.20f && aspect in 0.18f..1.10f && area >= 0.035f
            val overlapsFace = persons.any { iou(it.box, rect) > 0.18f }
            if (looksHuman && !overlapsFace) {
                persons += PersonObservation(
                    personId = "PERSON-${10000 + (obj.trackingId ?: index + 1)}",
                    box = rect,
                    confidence = 0.56,
        ...
        VisionResult(bitmap.width, bitmap.height, persons.distinctBy { it.personId }, tags, capturedAt)
    }
